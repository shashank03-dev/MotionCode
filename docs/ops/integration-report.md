...
- Data and auth foundation: Supabase schema, private `project-assets` storage, RLS policies, generated database types, auth middleware, and server/client Supabase helpers.
...
- Quality, security, and observability: CI workflow, lint/type/unit/e2e coverage, security headers/CSP, logger, analytics, observability helpers, audit exception tracking, and dependency cleanup.
...
- `npm audit --omit=dev --audit-level=moderate`: failed on known Next.js/PostCSS advisories documented in `docs/ops/audit-exceptions.md`.
...
- The clean production build emitted a Supabase Edge Runtime warning through `@supabase/ssr`; route modules that need trusted server behavior explicitly use Node runtime.
...
- Track the documented Next.js/PostCSS audit exception in a dedicated framework upgrade branch before paid production traffic.
